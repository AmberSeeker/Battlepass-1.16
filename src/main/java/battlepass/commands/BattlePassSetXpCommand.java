...
public class BattlePassSetXpCommand implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 2) {
            ...
            battlepassPlayer.setXp(xp);
            sender.sendMessage(Utils.toText("&aSet " + playerName + " XP to " + xp));
        } else {
            sender.sendMessage(Utils.toText("&cUsage: /setxp <player> <amount>"));
        }
        return true;
    }
}
